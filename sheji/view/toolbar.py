...
class Toolbar:
    ...
    def draw(self, current_shape_type, current_color, current_size):
        ...
        self.screen.blit(color_title, (20, self.color_button_y - 25))
        
        x = self.color_button_x
        for color in self.color_buttons:
            color_rect = pygame.Rect(x, self.color_button_y, self.color_button_size, self.color_button_size)
    ...
    def handle_click(self, pos, current_shape_type):
        ...
        if y >= self.color_button_y and y <= self.color_button_y + self.color_button_size:
            btn_x = self.color_button_x
            for color in self.color_buttons:
                color_rect = pygame.Rect(btn_x, self.color_button_y, self.color_button_size, self.color_button_size)
        ...
        if y >= self.action_button_y:
            btn_y = self.action_button_y
            for btn in self.action_buttons:
                btn_rect = pygame.Rect(10, btn_y, self.width - 20, self.button_height)
                if btn_rect.collidepoint(pos):
