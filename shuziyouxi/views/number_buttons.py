...
class NumberButtons(ctk.CTkFrame):
    ...
    def _create_widgets(self):
        self.time_frame = ctk.CTkFrame(self, corner_radius=10)
        self.time_frame.pack(fill="x", pady=10, padx=10)
    ...
    def set_options(self, options: list):
        ...
        for i, option in enumerate(options):
            btn = ctk.CTkButton(
                ...
                fg_color="#96CEB4",
                hover_color="#85BB9E",
                command=lambda n=option: self._on_number_click(n)
            )
            row = i // cols
            col = i % cols
    ...
    def set_time(self, seconds: int):
        color = "#E74C3C" if seconds <= 10 else "#6C757D"
        self.time_label.configure(text=f"⏱️ 剩余时间: {seconds}秒", text_color=color)
